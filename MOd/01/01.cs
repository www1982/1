    ...
    public static Dictionary<int, CellInfo> GetCellsInfo(List<int> cells)
    {
        ...
        foreach (var cell in cells)
        {
            ...
            var info = new CellInfo
            {
                ...
                DiseaseName = Db.Get().Diseases[Grid.DiseaseIdx[cell]].Id,
...
{
    // 1. 建造能力 (修正版)
    public static void Build(string buildingId, int cell)
    {
...
public class ApiServer
{
    ...
    private async void HandleRequest(HttpListenerContext context)
    {
        ...
        try
        {
            ...
            {
                JObject query = JObject.Parse(body);
                await MainThreadDispatcher.RunOnMainThread(() => GlobalActionExecutor.Build(query["buildingId"].Value<string>(), query["cell"].Value<int>()));
